...
$recoveryLabel = New-Label '本次故障恢复上限' 18 176 145 22
...
$maxDelayLabel = New-Label '最大等待（秒）' 18 260 145 22
...
function Get-StoppedStateText {
    ...
    if ($Reason -eq 'codex_restart_required') { return '未接入共享后台' }
...
function Update-RuntimeView {
    ...
    if (-not $running) {
        ...
        $serviceValue.Text = if (Get-SharedModeRequested (Read-JsonFile $configPath)) { '共享后台暂不可用' } else { '共享后台已关闭' }
        $serviceValue.ForeColor = [System.Drawing.Color]::DarkOrange
