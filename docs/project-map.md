...
| `scripts/build-provenance.ps1`, `scripts/build-info.json` | Source/EXE build identity checked before packaging and compared with the installed worker heartbeat. The JSON is generated by a successful build, not by packaging. |
| `scripts/build-provenance-smoke-test.ps1` | Proves missing provenance, changed source and stale EXEs are rejected using isolated fixtures. |
| `scripts/source/shared_recovery_windows.go` | Serialized worker-owned, expiring preparation requests from the launcher, separate from task retries and user preference. |
...
| `release/windows/close-codex.ps1`, `scripts/installer-close-smoke-test.ps1` | Read-only pre-install Desktop gate with bounded Retry/Cancel prompts, cancellation exit-code handling, and isolated regressions. Never terminates Codex. |
