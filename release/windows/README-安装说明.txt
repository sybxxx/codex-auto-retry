Codex Auto Retry 0.5.0 - Windows x64 安装说明
================================================
...
托盘设置
--------
双击托盘图标可以查看任务倒计时和队列，并设置后备重试文字、最多连续重试
次数（默认 5 次）、首次和最大等待时间及 Windows 通知。达到连续重试上限后，
该任务会停止自动尝试，也可以在列表中选择“重新开始”。

管理面板
--------
在 Codex 中输入“打开 Codex Auto Retry 管理面板”，可以查看倒计时、任务队列、
暂停开关、连续重试上限和等待时间，并修改后备重试文字。普通对话会先在原任务中
...
次数上限和等待规则自动重试。用户取消当前轮次或暂停目标时，自动重试会停止。
